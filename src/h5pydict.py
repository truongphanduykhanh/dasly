...
class DictFile(File):
    ...
    def load_dict(self, field=None, skipFields=[], getData=True):
        ...
        if isinstance(field, Dataset):
            ...
            self.__recursion_load(
                self if field is None else field, data, 0, getData)
        return data
            ...
            for key, val in tree.items():
                if isinstance(val, dict):
                    ...
                    try:
                        ...
                        # convert string array to str
                        if isinstance(val, np.ndarray) and val.size == 1 and val.dtype.char == 'U':
        ...
        if tree == None or not isinstance(tree, Group):  # siste nivå
            ...
            for key, val in tree.items():
                ...
                if isinstance(val, Group):
                    ...
                    parentdict[key] = {}
                    self.__recursion_load(
                        val, parentdict[key], depth+1, getData)
                else:
    ...
    def __print_info(self, key, val, depth, maxlength=16):
        ...
        if (len(val.shape) == 0 or len(val) <= maxlength):
            print("\t" * depth + ("%s (%s):" %
                  (key, str(val.dtype))).ljust(30) + str(val[()]))
        else:
            print("\t" * depth + ("%s (%s):" %
                  (key, str(val.dtype))).ljust(30) + str(val.shape))
