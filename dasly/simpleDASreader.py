...
def load_DAS_files(filepaths, chIndex=None, samples=None, sensitivitySelect=0,
        ...
        with h5pydict.DictFile(filepath, 'r') as f:
            ...
            if n == 0:
                ...
                if len(m['header']['channels'][chIndex]) == 0:
                    raise IndexError(
                        f'chIndexes {chIndex} not in data (nChannels = {nChs})')
                samples_max = (nSamples+1)*len(filepaths)
                channels_max = len(range(nChs)[chIndex]) if isinstance(
                    chIndex, slice) else len(chIndex)
    ...
    if sensitivitySelect >= 0:
        try:
            ...
            print('SensitivitySelect index %d not found in file.' %
                  sensitivitySelect)
            raise
        ...
        sensitivityUnits_out = [_combine_units(
            [sensitivityUnit, 'm']) for sensitivityUnit in m['header']['sensitivityUnits']]

    elif sensitivitySelect == -3 and userSensitivity is not None:  # use userdef sensitivity
        unit_out = _combine_units(
            [m['header']['unit'], userSensitivity['sensitivityUnit']], '/')
    ...
    if unwr and m['header']['spatialUnwrRange']:
        signalnd = unwrap(signalnd, m['header']
                          ['spatialUnwrRange']*sensitivity, axis=1)
    ...
    if integrate:
        ...
        if not any([u == 's' for u in re.findall(r"[\w']+", unit_new)]):
            ...
            warnings.warn(
                'Data unit %s is not differentiated. Integration skipped.' % unit_out, UserWarning)

    # create timeaxis
    nstart = 0 if samples.start is None else samples.start
    tstart = datetime.datetime.utcfromtimestamp(
        m['header']['time']+nstart*m['header']['dt'])
    nSamples, nCh = signalnd.shape
    t = create_time_axis(tstart, nSamples, m['header']['dt'])

    # create pandas dataframe
    meta = {key: m['header'][key] for key in ['dt', 'dx', 'gaugeLength', 'experiment',
                                              'dataType', 'dimensionRanges', 'dimensionUnits', 'dimensionNames', 'name']}
    ...
    # print('Loaded files '+str(files)+'  in %.1f s' %
    #       (datetime.datetime.now()-loadstart).total_seconds())
...
def find_DAS_files(experiment_path, start, duration, channels=None, datatype='dphi',
    ...
    for date_in_exp in dates_in_exp:
        ...
        if datetime_start.date() <= date1.date() <= datetime_stop.date():
            ...
            ffiddir = os.path.join(
                experiment_path, date_in_exp, datatype) if exppath_has_dates else experiment_path

            for ffid in sorted(os.listdir(ffiddir)):
                try:
                    ffidTime = str2datetime(
                        date_in_exp+' '+os.path.splitext(ffid)[0])
                except:
                    continue
    ...
    if len(ffidpaths) > 0:
        ...
        if datetime_start.year == 1900:  # replace the placeholder date 19000101 with date from headerTime
            datetime_start = datetime.combine(
                headerTime.date(), datetime_start.time())

        if load_file_from_start:
            startSample = 0
        else:
            startSample = max(
                0, int((datetime_start-headerTime).total_seconds()/header['dt']+.5))
        samples = slice(startSample, startSample+nSamples_to_be_read)

        if show_header_info:
            print('-- Header info file: %s --' %
                  os.path.basename(ffidpaths[0]))
            print('\tExperiment:            %s' % header['experiment'])
            print('\tFile timestamp:        %s' %
                  headerTime.strftime("%Y-%m-%d %H:%M:%S"))
            print('\tType of data:          %s, unit: %s' %
            ...
            print('\tSensitivities:         %s' % ','.join('%.2e %s' % (sens, unit)
        ...
        if channels is not None:
            ...
            channels_found, chIndex, _ = np.intersect1d(
                header_channels, channels, return_indices=True)
            if show_header_info:
                with np.printoptions(threshold=20):
                    print('\t%d channels requested:%s' %
                          (len(channels), channels))
                    print('\t%d channels found:    %s' %
                          (len(channels_found), channels_found))
        else:
            chIndex = None
    else:
        chIndex = None
        samples = None
        warnings.warn(
            'simpleDASreader.find_DAS_files did not find any files', UserWarning)
    if show_header_info:
        print('-----------------------------------')
...
def save_to_DAS_file(signal, filepath=None, datatype='processed'):
    ...
    utctimestamp = signal.meta['time'].replace(
    ...
    h5pydict.save(filepath, dict(fileVersion=fileVersion,
                  header=header, data=np.asfarray(signal, np.float32)))
...
class DASDataFrame(pd.DataFrame):
    ...
    def _set_dimensionRange(self):
        ...
        # remove subseqent indexes due to double differensiation
        d2chs[np.r_[False, np.logical_and(d2chs[:-1], d2chs[1:])]] = 0
        # where to split roi and create a new
        stopind = np.r_[np.where(d2chs != 0)[0]+1, len(chs)-1]
...
def convert_index_to_rel_time(signal):
    ...
    '''
    signal['t'] = (signal.index-signal.index[0]
                   ).total_seconds()  # seconds from start of data
...
def _combine_units(units, operator='*'):
    ...
    combinedUnit: str
        The combined unit.
    """
    knownUnits = ('rad', 'dt', 'strain', 'W', 'NormCts',
                  'm', 'K', 's', 'Hz', 'N', 'Pa', 'J')
...
def _fix_meta(meta, nSamples, nChannels):
    ...
    if 'dx' not in meta['header'] or 'gaugeLength' not in meta['header']:
        ...
        if 'dx' not in meta['header']:
            meta['header']['dx'] = dx_fiber / \
                meta['cableSpec']['fiberOverLength']
    ...
    if 'dataScale' not in meta['header']:
        meta['header']['dataScale'] = np.pi/2**29 / \
            meta['header']['dt']/meta['header']['gaugeLength']
    ...
    if 'dimensionRanges' not in meta['header']:
        ...
        # time dimension
        meta['header']['dimensionRanges'].update(dimension0={'min': 0, 'max': nSamples-1,
                                                             'size': nSamples,
        ...
        meta['header']['dimensionRanges'].update(dimension1={'min': cols['roiStart'], 'max': cols['roiEnd'],
    ...
    if 'sensitivity' in meta['header']:  # should be plural
        meta['header']['sensitivities'] = np.atleast_2d(
            meta['header'].pop('sensitivity'))
        meta['header']['sensitivityUnits'] = np.atleast_1d(
            meta['header'].pop('sensitivityUnit'))
    ...
    elif 'sensitivityUnits' in meta['header'] and meta['header']['sensitivityUnits'][0] == 'strain/s':
        meta['header']['sensitivityUnits'] = ['rad/(strain*m)']

    if 'unit' in meta['header']:
        meta['header']['unit'] = meta['header']['unit'].replace(
            '·', '*').replace('ε', 'strain')
    if 'sensitivityUnits' in meta['header']:
        meta['header']['sensitivityUnits'] = [u.replace(
            '·', '*').replace('ε', 'strain') for u in meta['header']['sensitivityUnits']]
